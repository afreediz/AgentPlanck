...
This module provides integration between external MCP servers and registry.
MCP tools are dynamically discovered and registered as tools.
"""
...
from tools import ToolResult, Registry, ToolsController
...
class MCPClient:
	"""Client for connecting to MCP servers and exposing their tools as actions."""
	...
	async def register_to_controller(
		...
		"""Register MCP tools as actions in the controller.

		Args:
			controller: controller to register actions to
		...
		logger.info(f"✅ Registered {len(self._registered_actions)} MCP tools from '{self.server_name}' as actions")

	def _register_tool_as_action(self, registry: Registry, action_name: str, tool: Any) -> None:
		"""Register a single MCP tool as a action.

		Args:
			registry: registry to register action to
		...
		if param_model:
			...
			async def mcp_action_wrapper(params: param_model) -> ToolResult:  # type: ignore[no-redef]
				"""Wrapper function that calls the MCP tool."""
				if not self.session or not self._connected:
					return ToolResult(error=f"MCP server '{self.server_name}' not connected", success=False)
			...
			async def mcp_action_wrapper() -> ToolResult:  # type: ignore[no-redef]
				"""Wrapper function that calls the MCP tool."""
				if not self.session or not self._connected:
					return ToolResult(error=f"MCP server '{self.server_name}' not connected", success=False)
	...
	def _format_mcp_result(self, result: Any) -> str:
		"""Format MCP tool result into a string for ToolResult.
	...
	def _json_schema_to_python_type(self, schema: dict) -> Any:
		...
		if schema.get('nullable', False):
			base_type = Union[base_type, None]

		# Handle arrays
		if json_type == 'array':
			items_schema = schema.get('items')
			if items_schema and isinstance(items_schema, dict) and 'type' in items_schema:
				# 🟢 RECURSE to get element type!
				item_type = self._json_schema_to_python_type(items_schema)
				return list[item_type]
			else:
				# ⚠️ Invalid or missing items type — fallback to untyped list
				return list[Any]
		...
		if json_type == 'object' and 'properties' in schema:
			...
			return create_model("DynamicNested", __base__=BaseModel, **fields)
